  ...
  template: `
    <div class="space-y-5">
      <div class="flex items-center justify-between pb-3 border-b border-slate-800">
        <h3 class="text-sm font-bold text-indigo-400 uppercase tracking-wider">Experience Section</h3>
        <div class="flex items-center gap-2">
          <button
            type="button"
            (click)="state.toggleSectionVisibility('experience')"
            ...
            [ngClass]="state.portfolio().experience.visible ? 'bg-slate-800 border-slate-700 text-slate-200 hover:bg-slate-700' : 'bg-rose-950/80 border-rose-800 text-rose-300 hover:bg-rose-900'"
            [title]="state.portfolio().experience.visible ? 'Hide Experience section' : 'Show Experience section'"
          >
            <app-icon [name]="state.portfolio().experience.visible ? 'eye' : 'eye-off'" [size]="14"></app-icon>
            <span>{{ state.portfolio().experience.visible ? 'Hide Section' : 'Show Section' }}</span>
          </button>
          <button
            (click)="addExperience()"
          ...
          >
            <app-icon name="plus" [size]="14"></app-icon> Add Experience
          </button>
        </div>
      </div>
